...
import numpy as np
from my_fun import hog_improve, non_image, print_result
...
def load_data(p, c):
    ...
    while i <= 5222:
        if i in non_image or i in size_image:
            ...
            fd, _ = hog(img, orientations=9, pixels_per_cell=(p, p), cells_per_block=(c, c),
                        block_norm='L2-Hys', visualize=True)
            features_1.append(fd.tolist())
...
for i in range(1):
    ...
    for j in range(1):
        ...
        svm = SVC(kernel='rbf', C=14, gamma=1.5)
        svm.fit(X_train, y_train)
        y_predict = svm.predict(X_test)
        acc, precision, recall = print_result(y_test, y_predict, 'p='+str(pp)+' c='+str(c))
        a.append(str(acc))
